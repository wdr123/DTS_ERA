...
title = "Average Test Accuracy for eye-gaze dataset across 5 seeds"
    ...
    if idx==2:
        ...
        for metric in ['Test Accuracy']:
            ...
            #                         facecolor=clrs[idx])
                ...
                # if idx == 0:
                #     ax.plot(plot_this - offset, label=f"w/ attention")
                #     ax.fill_between(np.arange(200), plot_this - offset - error, plot_this - offset + error, alpha=0.3, facecolor=clrs[idx])
                # else:
                #     ax.plot(plot_this - offset, label=f"w/o attention")
                #     ax.fill_between(np.arange(200), plot_this - offset - error, plot_this - offset + error, alpha=0.3, facecolor=clrs[idx])

                # if idx == 0:
                #     ax.plot(plot_this - offset, label=f"Read")
                #     ax.fill_between(np.arange(200), plot_this - offset - error, plot_this - offset + error, alpha=0.3, facecolor=clrs[idx])
                # else:
                #     ax.plot(plot_this - offset, label=f"Paint")
                #     ax.fill_between(np.arange(200), plot_this - offset - error, plot_this - offset + error, alpha=0.3, facecolor=clrs[idx])

                ax.plot(plot_this - offset, label=f"Eye-Gaze")
                ax.fill_between(np.arange(200), plot_this - offset - error, plot_this - offset + error, alpha=0.3, facecolor=clrs[idx])
...
# if not os.path.exists('../results/visualization/read_paint'):
#     os.mkdir('../results/visualization/read_paint')
if not os.path.exists('../results/visualization/eye_gaze'):
    os.mkdir('../results/visualization/eye_gaze')

# plt.savefig('../results/visualization/ablation/latent_dim.png')
plt.savefig('../results/visualization/eye_gaze/attention1.png')
