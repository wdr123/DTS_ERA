...
# scale = 1
decay = 0.95
model = MODEL(window_size= 8, std = std).to(device)
...
for epoch in range(500):
    ...
    test_dict = {
        'Epoch': epoch,
        'Test Accuracy (%)': train_reward * 100 / len(train_dl.dataset),
        'Test Action Loss': train_aloss / len(train_dl.dataset),
        'Test Location Loss': train_lloss / len(train_dl.dataset),
        'Test Baseline Loss': train_bloss / len(train_dl.dataset),
