...
class ASD_TD_CNP(nn.Module):
    ...
    def __init__(self,is_touch=True,is_gaze=True):
        ...
        self.touch_weight_mask = nn.Sequential(
            collections.OrderedDict([
                ("touchwei1", nn.Linear(latent_dim, num_ts*(gaze_inp+touch_inp))),
                ("touchwei1relu1", nn.ReLU()),
                ("touchwei2", nn.Linear(latent_dim, num_ts)),
                ("touchwei2soft2", nn.Softmax()),
            ])
        )

        self.gaze_weight_mask = nn.Sequential(
            collections.OrderedDict([
                ("gazewei1", nn.Linear(latent_dim, num_ts*(gaze_inp+touch_inp))),
                ("gazewei1relu1", nn.ReLU()),
                ("gazewei2", nn.Linear(latent_dim, num_ts)),
                ("gazewei2soft2", nn.Softmax())
            ])
        )
    ...
    def forward(self, touch_d, gaze_d, add_data=0):
        ...
        touch_weight_d = touch_d.view(bs, num_ts*touch_dim)
        gaze_weight_d = gaze_d.view(bs, num_ts*gaze_dim)
        ...
        touch_weight = self.touch_weight_mask(touch_weight_d)
        gaze_weight = self.gaze_weight_mask(gaze_weight_d)

        touch_emb = touch_emb.view(bs,num_ts,touch_emb.shape[-1])*touch_weight
        gaze_emb = gaze_emb.view(bs,num_ts,gaze_emb.shape[-1])*gaze_weight
...
class ASD_TD_CNP_LSTM(nn.Module):
    ...
    def __init__(self,is_touch=True,is_gaze=True):
        ...
        self.touch_weight_mask = nn.Sequential(
            collections.OrderedDict([
                ("touchwei1", nn.Linear(latent_dim, num_ts * (gaze_input_dim + touch_input_dim))),
                ("touchwei1relu1", nn.ReLU()),
                ("touchwei2", nn.Linear(latent_dim, num_ts)),
                ("touchwei2soft2", nn.Softmax()),
            ])
        ...
        self.gaze_weight_mask = nn.Sequential(
            collections.OrderedDict([
                ("gazewei1", nn.Linear(latent_dim, num_ts * (gaze_input_dim + touch_input_dim))),
                ("gazewei1relu1", nn.ReLU()),
                ("gazewei2", nn.Linear(latent_dim, num_ts)),
                ("gazewei2soft2", nn.Softmax())
            ])
